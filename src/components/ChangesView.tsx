...
export const ChangesView: React.FC<ChangesViewProps> = ({ currentBranch, localPath, owner, repoName, onRefresh }) => {
    ...
    const handleCommitPush = async () => {
        ...
        try {
            ...
            // Notify parent to refresh global status (including GlobalHeader)
            // Small delay to ensure Android filesystem flush
            setTimeout(() => {
                onRefresh();
            }, 500);
        ...
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--bg-color)' }}>
            ...
            <div style={{ flex: 1, overflowY: 'auto' }}>
                {changedFiles.map((file, idx) => (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: '12px', padding: '14px 16px', borderBottom: '1px solid rgba(255,255,255,0.05)' }}>
                        ...
                        <span style={{ fontSize: '14px', flex: 1, fontWeight: 500, color: 'var(--text-color)', fontFamily: 'monospace' }}>{file.path}</span>
            ...
            <AnimatePresence>
                {changedFiles.length > 0 && (
                    <motion.div initial={{ y: 200 }} animate={{ y: 0 }} style={{ background: 'var(--surface-color)', borderTop: '1px solid var(--border-color)', padding: '16px', paddingBottom: '24px', boxShadow: '0 -8px 24px rgba(0,0,0,0.3)' }}>
